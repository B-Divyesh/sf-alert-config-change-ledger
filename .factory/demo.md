...
Demo state uses only keys beginning with `demo:alert-config-ledger:`. **Reset demo** replaces that state with the bundled sample. **Start for real** deletes the demo state and returns to the non-demo landing workflow. **Install the CLI** is a separate shortcut to the installation instructions and also deletes demo state. The sample is cached by the site service worker and reloads after the browser goes offline.
