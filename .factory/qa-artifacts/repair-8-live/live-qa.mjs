...
const out = '.factory/qa-artifacts/repair-8-live';
...
await page.screenshot({ path: `${out}/landing-desktop-1440.png`, fullPage: true });
const actionBox = await page.getByRole('link', { name: 'Try it with sample data' }).boundingBox();
evidence.firstRead = { actionBox, viewport: page.viewportSize(), fullyVisible: Boolean(actionBox && actionBox.y + actionBox.height <= 900) };
...
evidence.keyboard.demoOutline = await page.evaluate(() => getComputedStyle(document.activeElement).outline);
await page.keyboard.press('Enter');
...
const missingResponse = await missingPage.goto(`${base}/missing-tape`, { waitUntil: 'networkidle' });
const missingAxe = await new AxeBuilder({ page: missingPage }).analyze();
evidence.routes['/missing-tape'] = {
  status: missingResponse?.status(),
  title: await missingPage.title(),
  h1: await missingPage.locator('h1').textContent(),
  axeSeriousCritical: missingAxe.violations.filter(item => ['serious', 'critical'].includes(item.impact || '')).map(item => item.id),
};
...
evidence.demo = {
  ...
  downloadedMatches: report.matched_routes,
...
evidence.demo.exitStorage = await demo.evaluate(() => ({
...
await offlinePage.screenshot({ path: `${out}/demo-mobile-offline.png`, fullPage: true });
await offlineContext.setOffline(false);

const mobileContext = await browser.newContext({ viewport: { width: 390, height: 844 } });
const mobile = await mobileContext.newPage();
await mobile.goto(`${base}/demo`, { waitUntil: 'networkidle' });
await mobile.screenshot({ path: `${out}/demo-mobile-390.png`, fullPage: true });
...
await mobile.screenshot({ path: `${out}/landing-mobile-390.png`, fullPage: true });
...
await writeFile(`${out}/live-qa.json`, JSON.stringify(evidence, null, 2));
